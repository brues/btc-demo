...
import btcdemo.btcdemo.security.SignatureUtils;
...
public class Transaction {
	...
	public void sign(String privateKey, Transaction prevTx) {
		...
		String sign = "";
		try {
			sign = SignatureUtils.sign(txClone.hash().getBytes(), privateKey);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean verify(Transaction prevTx) {
		...
		try {
			result = SignatureUtils.verify(txClone.hash().getBytes(), txIn.getPublicKey(), txIn.getSignature());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	 ...
	 */
	public String hash() {
		return Sha256Utils.sha256(JSON.toJSONString(this));
	}
